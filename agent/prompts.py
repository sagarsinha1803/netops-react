...
DEEP_CHECK_PROMPT = (
    ...
    "cannot be reached. Enumerate the routing contexts on the device -- `show "
    "vrf all`, `show ip vrf`, `show routing-instance` -- and look the "
    ...
    "hop and an egress interface; the neighbour on that interface is named by "
    "`show cdp neighbors interface <intf> detail` or `show lldp neighbors "
    "<intf> detail`; that is hop 1, with the interface it leaves by. Say which "
    "commands each hop came from.\n"
    " - RESOLVE RECURSIVELY. A next hop that is not directly connected is "
    "itself reached through another route: look IT up -- `show route "
    "<next-hop>` -- and keep going until the route is a connected interface. "
    ...
    " - MPLS and L3VPN: the label stack is the path. `show mpls forwarding-"
    "table <dst>`, `show bgp vpnv4 unicast vrf <vrf> <dst>` for the label and "
    "the remote PE, then follow that PE through the underlay -- `show route "
    "<remote-PE>` and a bounded traceroute to it. Transit LSRs that answer "
    ...
    "traffic would: `ping <dst> source <intf>`, `ping vrf <vrf> <dst>`. A "
    "probe from the wrong source address tests a route nobody uses.\n"
    ...
    " - `show vrf` prints a HEADER row. On NX-OS it is \'VRF-Name VRF-ID "
    "State Reason\'. Those are column titles, not VRFs. Never treat "
    ...
    "next command -- `show ip route vrf all <dst>`, or one VRF at a time if "
    "the platform has no all-VRF form. Listing the VRFs and then stopping "
    ...
    "probe it in that context -- `ping <dst> vrf management` -- rather than "
    "concluding the destination is unreachable from a default-table miss. "
    ...
    " - The forwarding table is the authority on what the hardware will do: "
    "`show forwarding ipv4 route <dst>` on NX-OS, `show cef <dst>` on IOS-XR "
    "and IOS-XE. A prefix present there with a real egress interface means "
    "the hardware can send it.\n"
    " - Confirm the neighbour is who you think: `show cdp neighbors interface "
    "<intf> detail`, or LLDP where CDP is off. Then cross-check the MAC it "
    "reports against `show ip arp | include <next-hop>`. When the two agree, "
    "the Layer 2 adjacency is proved, not assumed.\n"
    " - An EMPTY `show mac address-table` for a routed interface is normal, "
    "not a fault: a Layer 3 interface has no switching entries.\n"
    ...
    "SYNTAX THAT GETS REJECTED, by platform:\n"
    " - NX-OS traceroute takes few options: if `traceroute <dst> timeout 2 "
    "probe 3` is rejected, use plain `traceroute <dst>` or `traceroute <dst> "
    "vrf <vrf>`. Its route lookups are `show ip route <dst>`, `show ip route "
    "vrf all <dst>`, `show forwarding ipv4 route <dst>`.\n"
    " - IOS-XR: `show route <dst>`, `show route vrf all <dst>`, `show cef "
    "<dst>`, `traceroute vrf <vrf> <dst>`.\n"
    " - A rejected command is not a dead end: read the error, correct the "
    "syntax for THIS platform, and carry on. Three attempts at the same "
    "question, then move down the ladder and say which command the platform "
    "refused.\n"
